...
def create_entitlements(fname, profile, profile_uid, app_id, distribution):
    allow = 'true'
    aps = 'development'

    if distribution:
        allow = 'false'
        if 'aps-environment' in profile:
            aps = 'production'
        else:
            aps = ''
...
def make_entitlements(app_dir, distribution):
    ...
    message("App ID: %s.%s" % (profile_uid, app_id))
    fname = 'temp.xcent'
    create_entitlements(fname, profile, profile_uid, app_id, distribution)
...
def sign_local(app_dir, sign_id, distribution):
    ...
    if distribution:
        message("signing for App Store (Distribution entitlements)")
    else:
        message("NOT signing for App Store (Developer entitlements)")

    ents = make_entitlements(app_dir, distribution)
    if sys.platform == 'darwin':
        if not os.path.exists(codesign_allocate):
            error("codesign_allocate missing: %s" % codesign_allocate)
...
"""
    ...
    if options.dist:
        message("signing for App Store")
    else:
        message("NOT signing for App Store")

    if options.dist and not profile:
